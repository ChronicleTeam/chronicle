...
async fn create_field(
    ...
    if let FieldOptions::Decimal { number_precision: Some(number_precision), .. } = &mut create_field.options {
        *number_precision = u32::max(*number_precision, 1)
    }

    let field_id = query::insert_field(&pool, table_id, create_field.name, create_field.options)
        .await
        .on_constraint("meta_field_table_id_name_key", |_| {
            ApiError::unprocessable_entity([("field", "field name already used for this table")])
        })?;
...
async fn delete_field() -> ApiResult<()> {
    todo!()
}

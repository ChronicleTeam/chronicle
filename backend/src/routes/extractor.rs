...
impl AuthUser {
    ...
    async fn from_authorization(auth_header: HeaderValue) -> Result<Self, ApiError> {
        ...
        let hmac = Hmac::<Sha384>::new_from_slice(config.unwrap().hmac_key.as_bytes())
            .expect("HMAC-SHA-384 can accept any key length");
    ...
    S: Send + Sync,
{
    ...
    fn from_request_parts(
        ...
        let auth_header = req.headers.get(AUTHORIZATION).ok_or(ApiError::Unauthorized);

        Self::from_authorization(auth_header.expect("TEST").clone())
    }
}
...
//     ) -> impl Future<Output = Result<Self, Self::Rejection>> + Send {
//         let auth_header = req.headers.get(AUTHORIZATION).ok_or(ApiError::Unauthorized);
//         AuthUser::from_authorization(auth_header.expect("TEST2").clone())
//     }
// }

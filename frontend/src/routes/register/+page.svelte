    ...
    <div class="flex flex-col items-center bg-white p-6 rounded-lg shadow-lg w-80">
      ...
      <form on:submit|preventDefault={handleRegister} class="w-full">
        ...
        <div class="mb-4">
          ...
          <input type="password" id="password" bind:value={password} placeholder="Enter a password" class="w-full p-2 mt-1 border rounded-md" required />
        </div>
  
        <div class="mb-6">
          <label for="confirmPassword" class="block text-sm font-semibold">Confirm Password</label>
          <input type="password" id="confirmPassword" bind:value={confirmPassword} placeholder="Confirm your password" class="w-full p-2 mt-1 border rounded-md" required />
        </div>
        ...
        <button type="button" on:click={() => goto("/")} class="w-full mt-3 py-2 bg-gray-300 text-black hover:bg-gray-400 rounded-md transition-all duration-300">
          Back to Login
        </button>
      </form>
    </div>

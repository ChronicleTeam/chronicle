...
// Table methods
it("getTables calls GET with /tables", async () => {
...
it("getTableChildren calls GET with table id", async () => {
(GET as any).mockResolvedValueOnce([table]);
...
it("postCreateTable calls POST with payload", async () => {
...
it("postImportTable routes to csv endpoint", async () => {
...
it("postImportTable routes to excel endpoint", async () => {
const file = new File([""], "f.xlsx", { type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" });
(POST_FORM as any).mockResolvedValueOnce(table);
await postImportTable(file as any);
expect(POST_FORM).toHaveBeenCalledWith("/tables/excel", expect.any(FormData));
});

it("postImportTable throws for unsupported type", async () => {
...
it("postExportTable calls POST_FORM with correct endpoint", async () => {
...
it("patchTable calls PATCH with table id", async () => {
...
it("deleteTable calls DELETE with table id", async () => {
...
// Field methods
it("getFields calls GET and hydrates data", async () => {
...
it("postField calls POST with field payload", async () => {
...
it("patchField calls PATCH with field id", async () => {
(PATCH as any).mockResolvedValueOnce(field);
const res = await patchField(field as any);
expect(PATCH).toHaveBeenCalledWith(`/tables/${field.table_id}/fields/${field.field_id}`, {
name: "f1",
field_kind: "string"
});
expect(res).toEqual(field);
...
it("deleteField calls DELETE with field id", async () => {
(DELETE as any).mockResolvedValueOnce(undefined);
...
// Entry methods
it("getTableData calls GET and hydrates data", async () => {
...
it("postEntries calls POST with mapped entries", async () => {
...
it("patchEntry calls PATCH with entry id", async () => {
(PATCH as any).mockResolvedValueOnce(entry);
const res = await patchEntry(table as any, entry as any);
expect(PATCH).toHaveBeenCalledWith(`/tables/${table.table_id}/entries/${entry.entry_id}`, {
cells: { f1: "val" }
});
expect(res).toEqual(entry);
});

it("deleteEntry calls DELETE with entry id", async () => {
(DELETE as any).mockResolvedValueOnce(undefined);
